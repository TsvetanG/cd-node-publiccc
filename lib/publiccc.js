...
var Chaincode = class {
	...
	async pushClientData(stub, args) {
		...
		let channel = '';
		let result = await stub.invokeChaincode( chaincodeName, [ Buffer.from('push'), Buffer.from(clientId) , Buffer.from(privateData.toString('utf8')) ], channel );
	...
	async query(stub, args) {
		...
		while (true) {
			...
			let privateData = fetchPrivateData(stub, clientID, fileID);
	...
	fetchPrivateData(stub, clientID, fileID) {
		return 'Private Data hardcoded';
	}
